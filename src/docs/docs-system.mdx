...
of that sub category in the nav. If there's no `index.mdx` file, then the folder
name is used as the sub category name. 

The name of the main docs nav is controlled in the `_data.yml` file at the top 
of the instance (in our case, `docs/_data.yml`) in the `docs.title` setting.
...
directories appear in the categories, adjust their individual `menu.order` 
settings.

The default sort order is ascending (`/docs/index.mdx` has a value of -1 to stay on top).
